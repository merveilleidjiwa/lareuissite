...
<head>
    ...
    <title>Inscription Admin - La Réussite</title>
...
<body class="auth-page min-h-screen flex bg-gradient-to-br from-gray-900 via-gray-800 to-[#1a3d2e]">

    <div class="flex-1 flex flex-col lg:flex-row min-h-screen">
        <!-- Côté gauche : branding admin -->
        <div class="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-[#27ae60]/20 to-[#1e8449]/30 p-16 flex-col justify-between relative overflow-hidden border-r border-white/5">
            <div class="absolute inset-0">
                <div class="absolute top-20 left-20 w-64 h-64 rounded-full bg-[#27ae60]/10 blur-3xl"></div>
            ...
            <div class="relative space-y-6">
                <h2 class="text-4xl font-bold text-white leading-tight">Espace<br><span class="text-[#27ae60]">Administration</span></h2>
                <p class="text-gray-400 text-lg max-w-sm">Créez votre compte administrateur pour gérer produits, commandes et promos.</p>
                <div class="flex gap-4 text-gray-500">
                    <span><i class="fas fa-boxes text-[#27ae60] mr-2"></i> Gestion produits</span>
                    <span><i class="fas fa-shopping-cart text-[#27ae60] mr-2"></i> Commandes</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex-1 flex items-center justify-center p-6 lg:p-12">
            <div class="w-full max-w-md">
                <a href="{{ url('/') }}" class="lg:hidden flex items-center gap-3 mb-8 text-white">
                    ...
                    <div class="flex flex-col justify-center"><div class="flex items-baseline"><span class="text-2xl font-brand text-[#27ae60] leading-none">La Réussite</span><span class="font-black uppercase text-xs text-gray-500 ml-1">Admin</span></div><span class="text-[0.45rem] text-[#F9A825] uppercase tracking-[0.25em] font-black pl-1 mt-0.5">Agronomique</span></div>
                </a>

                <div class="bg-white rounded-[32px] shadow-2xl border border-gray-100 p-8 lg:p-10">
                    <h1 class="text-2xl font-bold text-gray-800 mb-2">Créer un compte Admin</h1>
                    <p class="text-gray-500 text-sm mb-8">Inscrivez-vous pour accéder au tableau de bord</p>
                    ...
                    <?php if ($message === "success"): ?>
                        <div class="bg-green-50 border border-green-200 text-green-800 p-5 rounded-2xl mb-6 text-center animate-fade-up">
                            ...
                            <p class="font-bold mb-3">Compte admin créé !</p>
                            <p class="text-sm text-gray-600 mb-4">Connectez-vous maintenant avec vos identifiants.</p>
                            <a href="{{ url('admin') }}" class="inline-block bg-[#27ae60] text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-[#219150] transition-colors">
                    ...
                    <form method="POST" class="space-y-5">
                        <div>
                            ...
                            <input type="text" name="nom" required placeholder="Ex: Merveille Kouassi" class="input-auth" value="<?php echo htmlspecialchars($_POST['nom'] ?? '', ENT_QUOTES); ?>">
                        </div>
                        <div>
                        ...
                        <button type="submit" class="btn-auth mt-2">
                            <i class="fas fa-user-shield mr-2"></i> Créer mon compte Admin
                        </button>
                    </form>

                    <p class="text-center mt-8 text-gray-500 text-sm">
                        Déjà un compte ? <a href="{{ url('admin') }}" class="text-[#27ae60] font-bold hover:underline">Se connecter</a>
                    </p>
                    <?php endif; ?>
                </div>

                <a href="{{ url('admin') }}" class="block text-center mt-6 text-gray-500 hover:text-[#27ae60] text-sm transition-colors">
                    <i class="fas fa-arrow-left mr-1"></i> Retour à la connexion admin
                </a>
            </div>
        </div>
    </div>
